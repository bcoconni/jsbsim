...
FGAngles::FGAngles(FGFCS* fcs, Element* element) : FGFCSComponent(fcs, element)
{
  ...
  if (element->FindElement("target_angle") ) {
    ...
    log << "Target angle is required for Angles component: " << Name << "\n";
    throw log;
  }
  ...
  if (element->FindElement("source_angle") ) {
    ...
    XMLLogException log(fcs->GetExec()->GetLogger(), element);
    log << "Source angle is required for Angles component: " << Name << "\n";
    throw log;
  }
  ...
  if (!unit.empty()) {
    if      (unit == "DEG") output_unit = 180.0/M_PI;
      ...
      XMLLogException log(fcs->GetExec()->GetLogger(), element);
      log << "Unknown unit " << unit << " in angle component, " << Name << "\n";
      throw log;
    }
  } else {
