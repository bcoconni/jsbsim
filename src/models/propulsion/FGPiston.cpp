  ...
  standard_pressure(101320.73)
{
  ...
  while((table_element = el->FindNextElement("table")) != 0) {
    ...
    try {
      if (name == "COMBUSTION") {
        Lookup_Combustion_Efficiency = make_unique<FGTable>(PropertyManager, table_element);
      } else if (name == "MIXTURE") {
        Mixture_Efficiency_Correlation = make_unique<FGTable>(PropertyManager, table_element);
      } else {
        XMLLogException err(Logger, table_element);
        err << "Unknown table type: " << name << " in piston engine definition.\n";
        throw err;
      }
    } catch (XMLLogException&) {
      throw;
    } catch (LogException& e) {
      throw XMLLogException(e, table_element);
    } catch (BaseException& e) {
      XMLLogException err(Logger, table_element);
      err << "Error loading piston engine table:" << name << ".\n" << e.what() << "\n";
      throw err;
    }
  }
...
// Default tables if not provided in the configuration file
  if(Lookup_Combustion_Efficiency == 0) {
    ...
    Lookup_Combustion_Efficiency = make_unique<FGTable>(12);
  ...
  if( Mixture_Efficiency_Correlation == 0) {
    Mixture_Efficiency_Correlation = make_unique<FGTable>(15);
