    ...
    if (el->GetNumElements("tableData") > 1) {
      ...
      if (tableData) {
        ...
        if (nLines > 1) {
          ...
          if (nLines == 0) {
            std::cerr << tableData->ReadFrom()
                      << "Missing data in <tableData>" << endl;
            throw BaseException("Missing data in <tableData>");
          }
          dimension = 1;
        }
        ...
        std::cerr << el->ReadFrom()
                  << "Missing <tableData> element." << endl;
        throw BaseException("Missing <tableData> element");
      }
    }
