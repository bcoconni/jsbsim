...
A Windows installer `JSBSim-0.99.116.dev1-setup.exe` is available in the [release section](https://github.com/JSBSim-Team/jsbsim/releases/tag/v0.99.116.dev1). It installs the 2 executables along with aircraft data and some example scripts:
...
Debian packages for Ubuntu Linux "Jammy" 22.04 LTS and "Noble" 24.04 LTS for 64 bits platforms are also available in the [JSBSim project release section](https://github.com/JSBSim-Team/jsbsim/releases/tag/v0.99.116.dev1). There are 3 packages for each platform:

* `JSBSim_0.99.116.dev1-2339.amd64.deb` which installs the executables `JSBSim` and `aeromatic`
* `JSBSim-devel_0.99.116.dev1-2339.amd64.deb` which installs the development resources (headers and libraries)
* `python3-JSBSim_0.99.116.dev1-2339.amd64.deb` which installs the Python module of JSBSim
