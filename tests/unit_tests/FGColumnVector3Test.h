...
public:
  ...
  void testOperatorParenthesis(void) {
    JSBSim::FGColumnVector3 v1(1., 0., -2.);

    TS_ASSERT_EQUALS(v1.Entry(1), v1(1));
    TS_ASSERT_EQUALS(v1.Entry(2), v1(2));
    TS_ASSERT_EQUALS(v1.Entry(3), v1(3));
  }
  ...
  // components
  void testAssignmentNoSideEffect(void) {
    JSBSim::FGColumnVector3 v;
    const JSBSim::FGColumnVector3 v1(1., 0., -2.);
  ...
  void testInitMatrixTwoParam(void) {
    JSBSim::FGColumnVector3 v;
  ...
  void testLhsScalarMultiplication(void) {
    const JSBSim::FGColumnVector3 v(1., 0.5, -2.);
    JSBSim::FGColumnVector3 v_twice = 2.0 * v;

    TS_ASSERT_EQUALS(v_twice(1), 2.0 * v(1));
    TS_ASSERT_EQUALS(v_twice(2), 2.0 * v(2));
    TS_ASSERT_EQUALS(v_twice(3), 2.0 * v(3));
  }
  ...
  void testRhsScalarMultiplication(void) {
    ...
    TS_ASSERT_EQUALS(v_twice, 2.0 * v);
  }
  ...
  void testAddition(void) {
    ...
    TS_ASSERT_EQUALS(v_twice + v, 3.0 * v);
  }
  ...
  void testSubtraction(void) {
    ...
    TS_ASSERT_EQUALS(v_twice - v, v);
  }

  void testAdditionAssignment(void) {
    const JSBSim::FGColumnVector3 v(1., 0.5, -2.);
    JSBSim::FGColumnVector3 v_twice(v);
  ...
  void testScalarDivisionAssignment(void) {
    const JSBSim::FGColumnVector3 v(1., 0.5, -2.);
    JSBSim::FGColumnVector3 v_half(v);
  ...
  void testDivisionByZeroAssignment(void) {
    ...
    v1 /= 0.0;
    TS_ASSERT_EQUALS(v1, v);
  }

  void testDotProduct(void) {
    const JSBSim::FGColumnVector3 v(1.0, -2.0, 0.5);
    const JSBSim::FGColumnVector3 X(1.0,0.0,0.0);
    const JSBSim::FGColumnVector3 Y(0.0,1.0,0.0);
  ...
  void testMagnitude(void) {
    const JSBSim::FGColumnVector3 v0;
    const JSBSim::FGColumnVector3 v(3.0, 4.0, 0.0);
